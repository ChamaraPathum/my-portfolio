...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && text === fullText) {
      ...
      }, isDeleting ? 50 : 100); // writing speed vs deleting speed
    }
    ...
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="max-w-6xl w-full mx-auto px-6 flex flex-col items-center justify-center text-center z-10">
        ...
        >
          ...
          <span className="text-sm tracking-wide text-gray-300">Available for Work</span>
        </motion.div>
        ...
        >
          Bridging the gap between stunning visual design and robust backend architecture. Let's build something extraordinary.
        </motion.p>
        ...
        >
          <a href="#projects" className="btn-3d px-10 py-4 rounded-xl text-white font-bold tracking-wide flex items-center gap-2 relative overflow-hidden group">
            <span className="relative z-10">Explore My Work</span>
            <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
          </a>
          <a href="#" className="card-3d px-8 py-4 rounded-xl text-white font-semibold flex items-center gap-2 group">
            <span className="group-hover:text-brand-accent transition-colors">Download CV</span>
            <Download size={18} className="group-hover:text-brand-accent transition-colors" />
          </a>
        </motion.div>

        {/* 3D Isometric Floating Elements */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden -z-10" style={{ perspective: "1000px" }}>
          <motion.div
            animate={{ 
              rotateX: [0, 360],
              rotateY: [0, 360],
            ...
            className="absolute top-[20%] left-[15%] w-24 h-24 border border-brand-accent/30 rounded-xl"
            style={{ transformStyle: "preserve-3d" }}
          >
            <div className="absolute inset-0 border border-brand-accent/30 rounded-xl" style={{ transform: "translateZ(48px)" }} />
            <div className="absolute inset-0 border border-brand-accent/30 rounded-xl" style={{ transform: "translateZ(-48px)" }} />
          </motion.div>
          <motion.div
            animate={{ 
            ...
            style={{ transformStyle: "preserve-3d" }}
          >
            <div className="absolute inset-0 border border-brand-violet/20 rounded-xl" style={{ transform: "translateZ(64px)" }} />
            <div className="absolute inset-0 border border-brand-violet/20 rounded-xl" style={{ transform: "translateZ(-64px)" }} />
          </motion.div>
        </div>
